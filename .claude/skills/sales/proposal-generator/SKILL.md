...
The built-in `proposal_fetch_api_data` paths are currently bugged in this build (Ahrefs returns `missing argument 'select'`; Local Falcon throws `toLowerCase`). Do NOT rely on them. Pull data through these routes and pass results into slides via `proposal_update_state`. Always use EXPLICIT `YYYY-MM-DD` dates.

- **Organic performance, top pages, marketing funnel, traffic projection** → Composio `GOOGLE_ANALYTICS_RUN_REPORT` (GA4 sessions/users/engagement/keyEvents by `pagePath` and by `yearMonth`) + Composio `GOOGLE_SEARCH_CONSOLE_SEARCH_ANALYTICS_QUERY` (clicks/impressions/position by `page` and `query`). If GA4 returns HTTP 403, use the Pipedream `google_analytics-run-report-in-ga4` fallback. These replace the Ahrefs-based traffic slides for sites that have no Ahrefs history.
- **Local Pack / SoLV / local competitors** → Local Falcon MCP (`listLocalFalconScanReports` → `getLocalFalconReport`); build the `local_pack_*` grid + `top_competitors` from the scan's `data_points`.
- **GBP reviews / posts** → Pipedream `google_my_business-*` and/or Local Falcon.
- **Ahrefs (DR, backlinks, 24-month traffic history)** → unavailable (bugged + not in Composio). Mark the traffic-decline / backlink-profile / 24-month-history slides `skipped` until Ahrefs is restored. The Investment slide stays omitted per the no-pricing-numbers rule.

A young, growing site will have little organic history — frame the trajectory as growth, not decline, and lean on the local + GSC story.
