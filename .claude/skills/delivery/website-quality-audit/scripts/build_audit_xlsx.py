...
def classify_page_type(url):
    ...
    if matches(path, MONEY):     return "Money Page"
    if matches(path, AUTHORITY): return "Authority Page"
    if matches(path, UTILITY):   return "Utility Page"
    if matches(path, CONTENT):   return "Content Hub"
    return "Blog Post"
...
for r in records:
    ...
    if not r["top_keyword"] and imp < 100: continue
...
target_candidates.sort(key=lambda t: -t["score"])
